...
type ContractTransactor interface {
	...
	EstimateGas(ctx context.Context, call gochain.CallMsg) (gas uint64, err error)
...
type ContractFilterer interface {
	...
	FilterLogs(ctx context.Context, query gochain.FilterQuery) ([]types.Log, error)

	// SubscribeFilterLogs creates a background log filtering operation, returning
	// a subscription immediately, which can be used to stream the found events.
	SubscribeFilterLogs(ctx context.Context, query gochain.FilterQuery, ch chan<- types.Log) (gochain.Subscription, error)
}
