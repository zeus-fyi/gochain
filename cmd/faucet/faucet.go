...
import (
	...
	"github.com/zeus-fyi/gochain/v4/goclient"
	"github.com/zeus-fyi/gochain/v4/les"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the GoChain protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for GoChain and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // GoChain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an GoChain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // GoChain protocol stack
	client *goclient.Client    // Client connection to the GoChain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the GoChain light client protocol
...
// close terminates the GoChain connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the GoChain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and GoChain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for GoChain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No GoChain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the GoChain address and profile URL.
...
// returning the username, avatar URL and GoChain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain GoChain address,
